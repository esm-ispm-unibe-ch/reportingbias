nmafunnel <- function(nma, small.values="good") {
  ...
  if(comp.10==0) {
    ...
    res <- list(num.comp.10studies=paste("There are", comp.10, "comparisons with at least than 10 studies"))
  }
  else {
      ...
      for (j in 1:ncol(t)) {
        if(t[i,j]>9 & i<j) {
          if (nma$sm=="OR" | nma$sm=="RR") {
            ma <- metabin(event1,n1,event2,n2, subset = nma$treat1==rownames(t)[i] & nma$treat2==colnames(t)[j], data=nma$data, sm=nma$sm)
